...
import { Select, SelectCityInput, SelectContainer, SelectOption } from './SearchStyles';
...
const SelectCity = () => {
    ...
    return (
        <Select>
            <SelectCityInput onClick={() => setExpanded(prev => !prev)}>
                <FontAwesomeIcon opacity={selectedLocation.cityName !== '' ? '1' : '.7'} color={primaryColor} fontSize={'20px'} icon={'fa-solid fa-location-dot'} /> <p>{selectedLocation.cityName !== '' ? `${selectedLocation.cityName}, ${selectedLocation.countryName}` : '¿A dónde vamos?'}</p>
                <FontAwesomeIcon icon={expanded ? 'fa-solid fa-chevron-up' : 'fa-solid fa-chevron-down'} />
            </SelectCityInput>
            {
                expanded &&
                <SelectContainer>
